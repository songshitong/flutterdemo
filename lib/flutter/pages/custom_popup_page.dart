...
class CustomPopup extends StatefulWidget {
  //初始popupX 初始位置在右侧贴边
  double? popupX = (WidgetsBinding.instance?.window.physicalSize.width ?? 0.0) /
...
class CustomPopupState extends State<CustomPopup>
  ...
  void initState() {
    ...
    _controller =
        AnimationController(vsync: this, duration: Duration(milliseconds: 400))
          ..addListener(() {
            setState(() {
              popupX = MediaQuery.of(context).size.width * _controller.value -
                  radius;
              //贴边 而不是进入屏幕一部分
              popupX = popupX!
                  .clamp(0.0, MediaQuery.of(context).size.width - radius * 2);
            });
          })
    ...
    //更新时处理左右侧radius
    double windowWidth = WidgetsBinding.instance?.window.physicalSize.width ??
        0 / (WidgetsBinding.instance?.window.devicePixelRatio ?? 1);
  ...
  Widget build(BuildContext context) {
    ///布局使用stack 优点：位置灵活，指定布局以外的地方是透明的，其他地方可以响应事件
    return Stack(
      children: <Widget>[
        Positioned(
          left: popupX!,
          ...
          child: GestureDetector(
              onTap: () {
                ...
                Navigator.of(context)
                    ?.push(FadeTransparentRoute(builder: (context) {
                  ...
                  print(
                      "push PopupListPage result $result popupX $popupX popupY $popupY");
//                  WidgetsBinding.instance.addPostFrameCallback((duration) {  加入监听会闪一下，感觉刷新多次
                  if (null != result) {
                    widget._count = result;
                  }
                  if (result > 0) {
                    widget.popup(widget.outContext,
                        popupX: popupX, popupY: popupY);
                  } else {
...
class _PopupChildOneState extends State<PopupChildOne>
    with SingleTickerProviderStateMixin {
  late AnimationController _controller;
  @override
  void initState() {
...
class PopupChildTwoDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    if (hasChild(childId1)) {
      ...
      positionChild(
          childId1, Offset(child1W, size.height / 2 - child1Size.height / 2));
    }
    if (hasChild(childId2)) {
      Size child2Size = layoutChild(childId2, BoxConstraints.loose(size));
      double child2W = child2Size.width;
      positionChild(childId2,
          Offset(child1W / 2, size.height / 2 - child2Size.height / 2));
    }
  }
...
class PopupChildThreeDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    if (hasChild(childId1)) {
      Size child1Size = layoutChild(childId1, BoxConstraints.loose(size));
      child1W = child1Size.width;
      positionChild(childId1,
          Offset(child1W / 2, size.height / 2 - child1Size.height / 2));
    }
    if (hasChild(childId2)) {
      Size child2Size = layoutChild(childId2, BoxConstraints.loose(size));
      double child2W = child2Size.width;
      positionChild(
          childId2, Offset(child1W, size.height / 2 - child2Size.height / 2));
    }
    if (hasChild(childId3)) {
      ...
      positionChild(
          childId3,
          Offset(size.width / 2 - child3W / 2,
              size.height / 2 - child3Size.height / 2));
    }
  }
  ...
  void performLayout(Size size) {
    ...
    if (hasChild(childId1)) {
      ...
      positionChild(
          childId1,
          Offset(
              child1W / 2, size.height / 2 - child1Size.height / 2 + offset));
    }
    if (hasChild(childId2)) {
      Size child2Size = layoutChild(childId2, BoxConstraints.loose(size));
      double child2W = child2Size.width;
      positionChild(childId2,
          Offset(child1W, size.height / 2 - child2Size.height / 2 + offset));
    }
    if (hasChild(childId3)) {
      Size child3Size = layoutChild(childId3, BoxConstraints.loose(size));
      double child3W = child3Size.width;
      positionChild(
          childId3,
          Offset(size.width / 2 - child3W / 2,
              size.height / 2 - child3Size.height + offset));
    }
    if (hasChild(childId4)) {
      ...
      positionChild(
          childId4,
          Offset(size.width / 2 - child4W / 2,
              size.height / 2 - child4Size.height / 2));
    }
  }
...
class PopupChildFiveDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    if (hasChild(childId3)) {
      Size child3Size = layoutChild(childId3, BoxConstraints.loose(size));
      double child3W = child3Size.width;
      positionChild(childId3,
          Offset(child3W / 2, size.height / 2 - child3Size.height + offset));
    }
    if (hasChild(childId4)) {
      Size child4Size = layoutChild(childId4, BoxConstraints.loose(size));
      double child4W = child4Size.width;
      positionChild(childId4,
          Offset(child4W, size.height / 2 - child4Size.height + offset));
    }
    if (hasChild(childId5)) {
      ...
      positionChild(
          childId5,
          Offset(size.width / 2 - child5W / 2,
              size.height / 2 - child5Size.height / 2));
    }
  }
...
class FadeTransparentRoute extends PageRoute {
  FadeTransparentRoute({
    ...
    this.barrierLabel = "FadeTransparentRoute",
    ...
    children = List<Widget>.generate(widget.count, (int index) {
      ...
      child = LayoutId(
          ...
          child: PopupListItem(
            ...
            onItemClose: () {
              print(
                  "onItemClose ==== index $index  children.length ${children.length}");
              //点击关闭，移除item
              ...
              if (children.length == 0) {
                ...
                setState(() {
                  print(
                      " after remove ==== children.length ${children.length} datas.length ${datas.length}");
                });
              }
            },
          ));
  ...
  void performLayout(Size size) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (hasChild(childId)) {
        ...
        if (top! + allHeight + itemSpace * (count - 1) + bottomSpace >
            windowHeight) {
          //倒放
          offset = Offset(
              left!,
