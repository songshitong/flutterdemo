...
//    size当前绘制区域大小。
    ...
    canvas.drawArc(Rect.fromLTRB(0, 0, 50, 50), 0, 2 * math.pi * 15 / 18, false, myPaint); //使用 2pi角度
    ...
    print("startX $startX  startY $startY  size.width/2 ${size.width / 2} size.height/2 ${size.height / 2}");
    //旋转后的角度
    ...
    canvas.saveLayer(
        Rect.fromCircle(center: Offset(size.width / 2, size.height / 2), radius: 60),
        Paint()
          ..color = Colors.red
    ...
    canvas.drawLine(Offset(0, size.height / 2), Offset(size.width, size.height / 2), myPaint);
    //写字
    //1 Paragraph
    ui.ParagraphBuilder builder =
        ui.ParagraphBuilder(ui.ParagraphStyle(textAlign: TextAlign.center, textDirection: TextDirection.ltr))
    ...
    ui.Paragraph paragraph = builder.build()..layout(ui.ParagraphConstraints(width: 100));
    canvas.drawParagraph(paragraph, Offset(size.width / 2, size.height / 2 - paragraph.height / 2));
    ...
    textPainter = TextPainter()
      ...
      ..ellipsis = "..." //超出显示
      ..maxLines = 1
      ..strutStyle = StrutStyle.fromTextStyle(TextStyle()) //构建一个提供的[textStyle]包含等效属性值的StrutStyle
      //设置多个text把前面的覆盖了
      ..text = TextSpan(text: "text from textPainter", style: TextStyle(color: Colors.black), children: <InlineSpan>[
        //recognizer 没有实际分发，不会生效，一般是RichText的RenderParagraph的handleEvent方法
        TextSpan(text: "inline text", recognizer: TapGestureRecognizer()..onTap = () => print('inline text onTap'))
      ]);

    //计算用于绘制文本的字形的可视位置 minWidth/maxWidth 长度限制
    textPainter.layout(minWidth: 0, maxWidth: 300);
    //返回从文本顶部到///给定类型的第一个基线的距离
    double baseDis = textPainter.computeDistanceToActualBaseline(TextBaseline.alphabetic);
    textPainter.paint(canvas, Offset(10, size.height / 2 - textPainter.size.height / 2 + 100));
